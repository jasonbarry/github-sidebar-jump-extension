(() => {
  ...
  try {
    const items = $$('.TimelineItem.js-comment-container, .js-timeline-item .js-comment .TimelineItem:first-child, .ajax-pagination-form')
    items.forEach(item => {
      ...
      const row = `
        <div style="margin-bottom:4px">
          <img height="16" src="${avatarURL}" style="border-radius:${isBot ? 3 : 16}px;vertical-align:text-bottom" width="16" />
          <a href="${href}" style="margin-left:2px">${userName}</a>
          <time style="color:#999;font-size:12px;margin-left:2px">${timestamp}</time>
          <span style="font-size:11px;">${uniqueEmojis.map(emoji => `<span>${emoji}</span>`).join('')}</span>
        </div>
      `
